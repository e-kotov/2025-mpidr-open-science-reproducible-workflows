...
Now you can try to install a new package to see if `{renv}` works as expected. In the next exercise, we will need the `{targets}` package, so we can try installing it:
...
Agree and wait for the `{targets}` package and it's dependencies to be installed.
